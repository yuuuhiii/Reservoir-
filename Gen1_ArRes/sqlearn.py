...
# 【上段】波形の予測結果（今までと同じ）
ax1.plot(y_test, label="Target (Ideal Square Wave)", linestyle='--', color='gray', alpha=0.7)
ax1.plot(y_pred, label=f"AI Prediction (MSE: {mse_score:.4f})", color='red')
ax1.set_title("Physical Reservoir Computing: Sine to Square Wave Task")
ax1.set_ylabel("Amplitude")
ax1.legend()
ax1.grid(True)

# 【下段】各ノードの重み（Weight）の可視化
nodes = ['N0', 'N1', 'N2', 'N3', 'N4', 'N5', 'N6(D4)', 'N7(D6)']
weights = model.coef_  # これがAIが計算した「各ノードの重要度」です

# 見やすいように、プラスの重みは青、マイナスの重みは赤で塗る
colors = ['blue' if w > 0 else 'red' for w in weights]
ax2.bar(nodes, weights, color=colors, alpha=0.7)

# ゼロのラインを引き立たせる
ax2.axhline(0, color='black', linewidth=1)
ax2.set_title("Readout Weights: Which node is working hardest?")
